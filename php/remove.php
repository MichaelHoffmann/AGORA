...
function removeNode( $node, $mapID, $output, $linkID, $userID, $mapType ) {
	...
	$nID      = mysqli_real_escape_string( $linkID, $attr["ID"] );
	...
	if ( $userID == $row["user_id"] or $mapType == 'cooperate' ) {
		...
		$retval = mysqli_query( $linkID, $uquery );
		if ( ! $retval ) {
...
function remove( $xmlin, $userID, $pass_hash ) {
	...
	$delMap = mysqli_real_escape_string( $linkID, $xml['remove'] );
	...
	} else if ( $mapClause == 0 or mysqli_num_rows( $resultID ) == 0 ) {
