...
function reset_passwordLink( $username, $secanswer ) {
	...
	if ( $secanswer != $secAnswer ) {
		nonmatchSecQs( $output );

		return $output;
	}
	// username-Uid // ticket // Time //
	$uid_escaped = mysqli_real_escape_string( $linkID, $uid );
	$query       = "INSERT INTO password_requests (user_id,time_request) VALUES ($uid_escaped,NOW())";
...
function savePassword( $ticket, $newpwd ) {
	...
	$newpwd = mysqli_real_escape_string( $linkID, $newpwd );
...
if ( $action == "searchuser" ) {
	...
	$username  = $_REQUEST['username']; //TODO: Change this back to a GET when all testing is done.
	$secanswer = $_REQUEST['secanswer']; //TODO: Change this back to a GET when all testing is done.
