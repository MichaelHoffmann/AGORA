 ...
 */
function load_project_maps( $proj_id ) {
	global $version;
	...
	$proj_id  = mysqli_real_escape_string( $linkID, $proj_id );
	$query    = "SELECT * FROM maps WHERE proj_id=$proj_id AND is_deleted=0 ORDER BY maps.title";
...
$proj_id = $_REQUEST['proj_id'];  //TODO: Change this back to a GET when all testing is done.
$output  = load_project_maps( $proj_id );
print( $output->asXML() );
